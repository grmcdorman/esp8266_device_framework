...
namespace grmcdorman::device
{
    ...
    MqttPublisher::MqttPublisher(const __FlashStringHelper *manufacturer, const __FlashStringHelper *model, const __FlashStringHelper *software_version, Client *client):
        Device(FPSTR(mqtt_name), FPSTR(mqtt_name)),
        ...
        last_update(F("Last update<script>window.addEventListener(\"load\", () => { periodicUpdateList.push(\"MQTT&setting=last_update\"); });</script>"), F("last_update"))
    {
        ...
        last_update.set_request_callback([this] (const InfoSettingHtml &)
        {
            ...
            {
                last_update.set(F("Never published."));
            }
        });
    }
    ...
    void MqttPublisher::setup()
    {
        ...
        topicAvailability = prefix.get() + "/" + identifier.get() + "/status";
        topicState = prefix.get() + "/" + identifier.get() + "/state";
    ...
    void MqttPublisher::reconnect()
    {
        ...
        Serial.print(F("Attempting to connect to MQTT server; current state: "));
    ...
    void MqttPublisher::publish_auto_config()
    {
        ...
        //!< @TODO Much of this should not be hard-coded.
